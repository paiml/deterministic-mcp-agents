...
impl RefactorFsm {
    ...
    async fn process_event(&mut self, event: RefactorEvent) -> Result<RefactorState, String> {
        ...
        let new_state = match (&self.state, event) {
            ...
            (RefactorState::Analyzing, RefactorEvent::AnalysisComplete(_)) => RefactorState::Planning,
            (RefactorState::Planning, RefactorEvent::PlanGenerated(_)) => RefactorState::Refactoring,
            (RefactorState::Refactoring, RefactorEvent::RefactorApplied(_)) => RefactorState::Testing,
...
async fn demonstrate_12_states() {
    ...
    let states = vec![
        ...
        "Refactoring", "Testing", "Validating", "Complete",
        "Error", "Rollback", "Cancelled", "Paused",
    ];
...
async fn demonstrate_async_handlers() {
    ...
    fsm.process_event(RefactorEvent::Start("main.rs".to_string())).await.unwrap();
    println!("  State after Start: {:?}", fsm.state);
    
    fsm.process_event(RefactorEvent::ParseComplete(1000)).await.unwrap();
...
async fn demonstrate_hooks() {
    ...
    fsm.process_event(RefactorEvent::Start("test.rs".to_string())).await.unwrap();
    fsm.process_event(RefactorEvent::ParseComplete(500)).await.unwrap();
}
...
async fn test_determinism() {
    ...
    for _ in 0..1000 {
        let mut fsm = RefactorFsm::new();
        fsm.process_event(RefactorEvent::Start("file.rs".to_string())).await.unwrap();
        fsm.process_event(RefactorEvent::ParseComplete(100)).await.unwrap();
...
async fn demonstrate_rollback() {
    ...
    fsm.process_event(RefactorEvent::Start("rollback_test.rs".to_string())).await.unwrap();
    fsm.process_event(RefactorEvent::ParseComplete(200)).await.unwrap();
    fsm.process_event(RefactorEvent::AnalysisComplete(vec!["issue1".to_string()])).await.unwrap();
...
mod tests {
    ...
    async fn test_error_transition() {
        let mut fsm = RefactorFsm::new();
        fsm.process_event(RefactorEvent::ErrorOccurred("test error".to_string())).await.unwrap();
