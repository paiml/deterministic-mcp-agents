...
async fn demonstrate_join_pattern() {
    ...
    let futures = vec![
        async { sleep(Duration::from_millis(10)).await; "Task 1" },
        async { sleep(Duration::from_millis(15)).await; "Task 2" },
        async { sleep(Duration::from_millis(20)).await; "Task 3" },
    ];
...
async fn demonstrate_cancellation() {
    ...
    let handle = tokio::spawn(async move {
        handle_request(1, child_token).await
    });
