...
impl Operation {
    pub fn execute(&self) -> Result<i64, CalculatorError> {
        match self {
            Operation::Add(a, b) => a.checked_add(*b).ok_or(CalculatorError::Overflow),
            Operation::Subtract(a, b) => a.checked_sub(*b).ok_or(CalculatorError::Overflow),
            Operation::Multiply(a, b) => a.checked_mul(*b).ok_or(CalculatorError::Overflow),
